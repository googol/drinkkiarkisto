...
import { renderFrontend, requireUser, requireAdmin, requireUserOrLoginFactory, requireAdminOrLoginFactory, urlencodedParser, getDrinkFromRequestBody } from './middleware';
...
export function configureRoutes(app, connectionString) {
  ...
  app.route('/')
    .get(renderFrontend, ::drinksController.showList);

  app.route('/drinks')
    .get(ifHasQueryParam('new',
      (req, res, next) => requireUserOrLogin(req, res, () => renderFrontend(req, res, () =>drinksController.showNewEditor(req, res, next))),
  ...
  app.route('/drinks/:drinkId')
    .get(ifHasQueryParam('edit',
      (req, res, next) => requireAdminOrLogin(req, res, () => renderFrontend(req, res, () => drinksController.showSingleEditor(req, res, next))),
  ...
  app.route('/profile')
    .get(renderFrontend, requireUserOrLogin, ::profileController.showProfilePage)
